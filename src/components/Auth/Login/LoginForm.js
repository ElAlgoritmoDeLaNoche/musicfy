...
function handlerErrors(code) {
  switch (code) {
    case "auth/wrong-password":
      toast.warning("El usuario o la contraseña son incorrectos.", { theme: "colored" });
      break;
    case "auth/too-many-requests":
      toast.warning("Has enviado demasiadas solicitudes de reenvio de email de confirmacion en muy poco tiempo.", { theme: "colored" });
      break;
    ...
    if (formOk) {
      ...
      firebase.auth().signInWithEmailAndPassword(formData.email, formData.password).then(response => {
        ...
        if (!response.user.emailVerified) {
          toast.warning("Para poder iniciar sesión primero debes verificar la cuenta.", { theme: "colored" })
        }
      }).catch(err => {
        handlerErrors(err.code)
      }).finally(() => {
